...
mod arbitrary {
    ...
    pub fn arb_erc20_token_symbol() -> impl Strategy<Value = ERC20TokenSymbol> {
        "ck[0-9A-Za-z_]{0,18}"
            .prop_map(|token_symbol| ERC20TokenSymbol::from_str(&token_symbol).unwrap())
    }
    ...
    pub fn arb_burn_memo() -> BoxedStrategy<BurnMemo> {
        prop_oneof![
            arb_burn_native_memo(),
            arb_burn_native_for_erc20_fee_memo(),
            arb_burn_erc20_memo()
        ]
        .boxed()
    }

    fn arb_burn_native_memo() -> impl Strategy<Value = BurnMemo> {
        arb_address().prop_map(|to_address| BurnMemo::Convert { to_address })
    }

    fn arb_burn_native_for_erc20_fee_memo() -> impl Strategy<Value = BurnMemo> {
        (
            arb_erc20_token_symbol(),
            arb_checked_amount_of(),
            arb_address(),
        )
            .prop_map(
                |(erc20_token_symbol, erc20_withdrawal_amount, to_address)| BurnMemo::Erc20GasFee {
                    erc20_token_symbol,
