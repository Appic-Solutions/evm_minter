...
mod withdrawal_transactions {
    ...
    mod record_withdrawal_request {
        ...
        use crate::state::transactions::tests::{
            ...
            erc20_withdrawal_request_with_index, gas_fee_estimate, transaction_receipt,
        };
        ...
        fn should_record_withdrawal_request() {
            ...
            test(native_withdrawal_request_with_index(LedgerBurnIndex::new(
                15,
            )));
            test(erc20_withdrawal_request_with_index(
        ...
        fn should_fail_recording_withdrawal_request_when_duplicate_ledger_burn_index() {
            fn test<R: Into<WithdrawalRequest> + Clone, S: Into<WithdrawalRequest> + Clone>(
                ...
                expect_panic_with_message(
                    ...
                    "duplicate Native ledger burn index",
                );
            ...
            let native_burn_index = LedgerBurnIndex::new(15);
            let erc20_burn_index = LedgerBurnIndex::new(1);
            test(
                native_withdrawal_request_with_index(native_burn_index),
                native_withdrawal_request_with_index(native_burn_index),
            );
            test(
                native_withdrawal_request_with_index(native_burn_index),
                erc20_withdrawal_request_with_index(native_burn_index, erc20_burn_index),
            );
            test(
                erc20_withdrawal_request_with_index(native_burn_index, erc20_burn_index),
                native_withdrawal_request_with_index(native_burn_index),
    ...
    mod record_created_transaction {
        ...
        use crate::state::transactions::tests::{
            create_and_record_transaction, create_and_record_twin_withdrawal_requests,
            create_twin_withdrawal_requests, erc20_withdrawal_request_with_index, gas_fee_estimate,
            native_withdrawal_request_with_index, DEFAULT_ERC20_MAX_FEE_PER_GAS,
        };
        use crate::state::transactions::Erc20Value;
        use crate::state::transactions::{create_transaction, WithdrawalTransactions};
        use crate::test_fixtures::expect_panic_with_message;
        ...
        fn should_fail_when_withdrawal_request_not_found() {
            ...
            let burn_index = withdrawal_request.native_ledger_burn_index();
            expect_panic_with_message(
        ...
        #[test]
        fn should_fail_when_mismatch_with_native_withdrawal_request() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let withdrawal_request = native_withdrawal_request_with_index(LedgerBurnIndex::new(15));
            transactions.record_withdrawal_request(withdrawal_request.clone());
        ...
        #[test]
        fn should_fail_when_mismatch_with_erc20_withdrawal_request() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let withdrawal_request = erc20_withdrawal_request_with_index(
            ...
            expect_panic_with_message(
                || {
                    transactions.record_created_transaction(
                        withdrawal_request.native_ledger_burn_index,
        ...
        proptest! {
            ...
            fn should_fail_when_nonce_wrong(current_nonce in any::<u64>(), nonce_drift in 1..=u64::MAX) {
                ...
                let mut rng = reproducible_rng();
                let [withdrawal_request] = create_and_record_twin_withdrawal_requests(&mut transactions, &mut rng);
                ...
                expect_panic_with_message(
                    || transactions.record_created_transaction(withdrawal_request.native_ledger_burn_index(), tx_with_wrong_nonce),
                    "nonce mismatch",
                );
            }
        }
        ...
        #[test]
        fn should_create_and_record_native_transaction() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let transaction_price = gas_fee_estimate();
            for i in 0..100_u64 {
                let ledger_burn_index = LedgerBurnIndex::new(15 + i);
                let withdrawal_request = native_withdrawal_request_with_index(ledger_burn_index);
                transactions.record_withdrawal_request(withdrawal_request.clone());
                let expected_tx_amount = withdrawal_request
                    ...
                    .checked_sub(
                        transaction_price
                            .clone()
                            .to_price(NATIVE_WITHDRAWAL_TRANSACTION_GAS_LIMIT)
                            .max_transaction_fee(),
                    )
                    .unwrap();
                ...
                assert_eq!(
                    ...
                    Eip1559TransactionRequest {
                        ...
                        gas_limit: NATIVE_WITHDRAWAL_TRANSACTION_GAS_LIMIT,
                        destination: withdrawal_request.destination,
        ...
        #[test]
        fn should_create_and_record_erc20_transactions() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let gas_fee_estimate = gas_fee_estimate();
            for i in 0..100_u64 {
                let native_ledger_burn_index = LedgerBurnIndex::new(3 * i);
                let erc20_ledger_burn_index = LedgerBurnIndex::new(7 * i);
                let withdrawal_request = erc20_withdrawal_request_with_index(
                ...
                assert_eq!(
                    ...
                    Eip1559TransactionRequest {
                        ...
                        max_fee_per_gas: DEFAULT_ERC20_MAX_FEE_PER_GAS,
                        gas_limit: ERC20_WITHDRAWAL_TRANSACTION_GAS_LIMIT,
        ...
        fn should_record_signed_transactions() {
            ...
            for withdrawal_request in withdrawal_requests {
                ...
                assert_eq!(
                    transactions
                        .sent_tx
                        .get_alt(&native_ledger_burn_index)
    ...
    mod create_resubmit_transactions {
        ...
        use crate::state::transactions::tests::{
            create_and_record_erc20_withdrawal_requests,
            create_and_record_native_withdrawal_requests, create_and_record_signed_transaction,
            create_and_record_transaction, create_and_record_twin_withdrawal_requests,
            double_and_increment, gas_fee_estimate, native_withdrawal_request_with_index,
            DEFAULT_ERC20_MAX_FEE_PER_GAS, DEFAULT_MAX_TRANSACTION_FEE,
        };
        use crate::state::transactions::{
        ...
        #[test]
        fn should_resubmit_native_transaction_with_updated_price() {
            ...
            let tests = vec![
                ...
                ParameterizedTest {
                    ...
                    resubmitted_native_tx_amount_deduction: Wei::ZERO,
                },
                ...
                ParameterizedTest {
                    ...
                    resubmitted_native_tx_amount_deduction: Wei::from(
            ...
            fn check_resubmit_tx<R: Into<WithdrawalRequest>>(
                ...
                assert_eq!(
                    resubmitted_txs,
                    vec![Ok((native_ledger_burn_index, expected_resubmitted_tx))]
                );
            }
            ...
            for test in &tests {
                ...
                check_resubmit_tx(
                    test,
                    native_withdrawal_request_with_index(native_ledger_burn_index),
                );
            }
        }

        #[test]
        fn should_resubmit_multiple_native_transactions() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let withdrawal_requests: [WithdrawalRequest; 100] =
                create_and_record_native_withdrawal_requests(&mut transactions);
            let initial_price = GasFeeEstimate {
                base_fee_per_gas: WeiPerGas::from(10_u8),
                max_priority_fee_per_gas: WeiPerGas::from(2_u8),
            };
        ...
        #[test]
        fn should_not_resubmit_erc20_transactions_unless_max_priority_fee_increases() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let withdrawal_requests: [WithdrawalRequest; 100] =
                create_and_record_erc20_withdrawal_requests(&mut transactions);
            let initial_price = GasFeeEstimate {
                base_fee_per_gas: WeiPerGas::from(10_u8),
                max_priority_fee_per_gas: WeiPerGas::from(2_u8),
            };
            ...
            let too_high_price = GasFeeEstimate {
                base_fee_per_gas: DEFAULT_ERC20_MAX_FEE_PER_GAS,
        ...
        struct ParameterizedTest {
            ...
            resubmitted_native_tx_amount_deduction: Wei,
        }
    }
    ...
    mod record_resubmit_transaction {
        ...
        use crate::state::transactions::tests::{
            create_and_record_signed_transaction, create_and_record_transaction,
            create_and_record_twin_withdrawal_requests, erc20_withdrawal_request_with_index,
            gas_fee_estimate, native_withdrawal_request_with_index, sign_transaction,
        };
        use crate::state::transactions::{
        ...
        fn should_replace_existing_resubmitted_transaction() {
            ...
            let native_ledger_burn_index = LedgerBurnIndex::new(15);
            let erc20_ledger_burn_index = LedgerBurnIndex::new(7);
            test(native_withdrawal_request_with_index(
                native_ledger_burn_index,
            ));
            test(erc20_withdrawal_request_with_index(
                native_ledger_burn_index,
        ...
        fn should_contain_only_last_transactions() {
            ...
            assert_eq!(
                vec![
                    (
                        &TransactionNonce::ZERO,
                        &first_withdrawal_request.native_ledger_burn_index(),
                        vec![&first_tx, &last_first_tx]
                    ),
                    (
                        &TransactionNonce::ONE,
                        &second_withdrawal_request.native_ledger_burn_index(),
                        vec![&second_tx]
                    ),
                ],
    ...
    mod sent_transactions_to_finalize {
        ...
        use crate::numeric::{TransactionCount, TransactionNonce};
        use crate::state::transactions::tests::{
            create_and_record_signed_transaction, create_and_record_twin_withdrawal_requests,
            resubmit_transaction_with_bumped_price,
        };
        use crate::state::transactions::{WithdrawalRequest, WithdrawalTransactions};
        ...
        proptest! {
            ...
            fn should_be_empty_when_no_transaction_to_finalize(finalized_tx_count in arb_checked_amount_of()) {
                ...
                let [withdrawal_request] = create_and_record_twin_withdrawal_requests(
                    &mut transactions,
                    &mut rng
                );
                assert_eq!(
        ...
        fn should_contain_transactions_to_finalize() {
            ...
            let [first_withdrawal, second_withdrawal, third_withdrawal] =
                create_and_record_twin_withdrawal_requests(&mut transactions, &mut rng);
            let first_native_ledger_burn_index = first_withdrawal.native_ledger_burn_index();
            let second_native_ledger_burn_index = second_withdrawal.native_ledger_burn_index();
            ...
            let hashes_0: BTreeMap<_, _> = vec![sent_tx_0_0, sent_tx_0_1]
                .iter()
                .map(|tx| (tx.hash(), first_native_ledger_burn_index))
                .collect();
            ...
            let hashes_1: BTreeMap<_, _> = vec![sent_tx_1_0, sent_tx_1_1, sent_tx_1_2]
                .iter()
                .map(|tx| (tx.hash(), second_native_ledger_burn_index))
                .collect();
    ...
    mod record_finalized_transaction {
        ...
        fn should_fail_when_sent_transaction_not_found() {
            ...
            let wrong_index = LedgerBurnIndex::from(native_ledger_burn_index.get() + 1);
            expect_panic_with_message(
                || {
                    transactions.record_finalized_transaction(
                        wrong_index,
        ...
        #[test]
        fn should_record_native_finalized_transaction_and_not_reimburse() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let native_ledger_burn_index = LedgerBurnIndex::new(15);
            let withdrawal_request: WithdrawalRequest =
                native_withdrawal_request_with_index(native_ledger_burn_index).into();
            transactions.record_withdrawal_request(withdrawal_request.clone());
            let created_tx = create_and_record_transaction(
                &mut transactions,
                withdrawal_request.clone(),
            ...
            let maybe_reimburse_request = transactions
                ...
                .find(|r| r.native_ledger_burn_index() == native_ledger_burn_index)
        ...
        #[test]
        fn should_not_reimburse_unused_transaction_fee_when_erc20_withdrawal_successful() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
        ...
        #[test]
        fn should_reimburse_tokens_when_erc20_withdrawal_fails() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let native_ledger_burn_index = LedgerBurnIndex::new(7);
            let erc20_ledger_burn_index = LedgerBurnIndex::new(7);
            let withdrawal_request = erc20_withdrawal_request_with_index(
            ...
            assert_eq!(
                ...
                btreemap! {
                    ReimbursementIndex::Erc20 {
                        native_ledger_burn_index,
                        ledger_id: withdrawal_request.erc20_ledger_id,
                        erc20_ledger_burn_index } =>
        ...
        #[test]
        fn should_record_finalized_transaction_and_reimburse_unused_tx_fee_when_native_withdrawal_fails(
        ) {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let withdrawal_request = native_withdrawal_request_with_index(LedgerBurnIndex::new(15));
            transactions.record_withdrawal_request(withdrawal_request.clone());
            let native_ledger_burn_index = withdrawal_request.ledger_burn_index;
            let created_tx = create_and_record_transaction(
                &mut transactions,
                withdrawal_request.clone(),
                gas_fee_estimate(),
            ...
            let reimbursement_request = transactions
                ...
                .get(&native_reimbursement_index)
    ...
    mod transaction_status {
        ...
        use crate::state::transactions::tests::{
            ...
            native_withdrawal_request_with_index, withdrawal_transactions::withdrawal_flow,
        };
        use crate::state::transactions::{
        ...
        #[test]
        fn should_have_finalized_reimbursed_status_for_native_withdrawal() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let withdrawal_request = native_withdrawal_request_with_index(LedgerBurnIndex::new(15));
            let native_ledger_burn_index = withdrawal_request.ledger_burn_index;
            let receipt = withdrawal_flow(
        ...
        #[test]
        fn should_have_finalized_reimbursed_status_for_erc20_withdrawal() {
            let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
            let withdrawal_request = erc20_withdrawal_request_with_index(
                LedgerBurnIndex::new(15),
                LedgerBurnIndex::new(7),
            ...
            let erc20_reimbursement_index = ReimbursementIndex::Erc20 {
                native_ledger_burn_index: withdrawal_request.native_ledger_burn_index,
                ledger_id: withdrawal_request.erc20_ledger_id,
                erc20_ledger_burn_index: withdrawal_request.erc20_ledger_burn_index,
            };
            transactions.record_finalized_reimbursement(
...
mod oldest_incomplete_withdrawal_timestamp {
    ...
    fn should_return_the_min_of_two_requests() {
        let mut transactions = WithdrawalTransactions::new(TransactionNonce::ZERO);
        let mut rng = reproducible_rng();
        let [mut first_request, mut second_request] = create_twin_withdrawal_requests(&mut rng);
...
mod native_withdrawal_request {
    use crate::numeric::LedgerBurnIndex;
    use crate::state::transactions::tests::native_withdrawal_request_with_index;
...
mod create_transaction {
    ...
    use crate::state::transactions::tests::{
        erc20_withdrawal_request_with_index, gas_fee_estimate, native_withdrawal_request_with_index,
    };
    use crate::state::transactions::{
    ...
    proptest! {
        ...
        fn should_fail_when_amount_does_not_cover_transaction_fees(withdrawal_amount in any::<u64>()) {
            ...
            let native_withdrawal_request = NativeWithdrawalRequest {
                withdrawal_amount: insufficient_amount,
                ..native_withdrawal_request_with_index(native_ledger_burn_index)
            };
            let result = create_transaction(
                &native_withdrawal_request.clone().into(),
            ...
            prop_assert_eq!(
                ...
                Err(CreateTransactionError::InsufficientTransactionFee {
                    native_ledger_burn_index,
                    allowed_max_transaction_fee: native_withdrawal_request.withdrawal_amount,
            ...
            let erc20_withdrawal_request = Erc20WithdrawalRequest {
                max_transaction_fee: insufficient_amount,
                ..erc20_withdrawal_request_with_index(native_ledger_burn_index, LedgerBurnIndex::new(2))
            };
            let result = create_transaction(
                &erc20_withdrawal_request.clone().into(),
                TransactionNonce::TWO,
                gas_fee,
                gas_limit,
                EvmNetwork::Sepolia,
            ...
            prop_assert_eq!(
                ...
                Err(CreateTransactionError::InsufficientTransactionFee {
                    native_ledger_burn_index,
                    allowed_max_transaction_fee: erc20_withdrawal_request.max_transaction_fee,
    ...
    proptest! {
        ...
        fn should_create_erc20_withdrawal_transaction(max_transaction_fee in 4_652_229_101_896_296_u128..=u128::MAX) {
            ...
            let native_ledger_burn_index = LedgerBurnIndex::new(15);
            let erc20_ledger_burn_index = LedgerBurnIndex::new(2);
            let withdrawal_request = Erc20WithdrawalRequest {
...
mod withdrawal_flow {
    ...
    use crate::state::transactions::{create_transaction, EvmNetwork, WithdrawalTransactions};
    use crate::withdraw::estimate_gas_limit;
    use proptest::proptest;
        ...
        proptest!(|(gas_fee_estimate in arb_gas_fee_estimate(), transaction_count in arb_checked_amount_of())| {
            ...
            for request in withdrawal_requests {
                ...
                if let Ok(created_tx) = create_transaction(
                    ...
                    wrapped_txs.borrow_mut().record_created_transaction(request.native_ledger_burn_index(), created_tx);
                }
            }
...
pub mod arbitrary {
    ...
    pub fn arb_withdrawal_request() -> BoxedStrategy<WithdrawalRequest> {
        prop_oneof![
            arb_native_withdrawal_request().prop_map(|req| req.into()),
            arb_erc20_withdrawal_request().prop_map(|req| req.into()),
        ]
        .boxed()
    }

    fn arb_native_withdrawal_request() -> impl Strategy<Value = NativeWithdrawalRequest> {
        (
    ...
    fn arb_erc20_withdrawal_request() -> impl Strategy<Value = Erc20WithdrawalRequest> {
        (
            arb_checked_amount_of(),
            arb_checked_amount_of(),
            arb_address(),
...
fn native_withdrawal_request_with_index(
...
/// Create a mix of Native and ERC20 withdrawal requests and record them.
fn create_and_record_twin_withdrawal_requests<const N: usize, R: Rng>(
    transactions: &mut WithdrawalTransactions,
    rng: &mut R,
) -> [WithdrawalRequest; N] {
    let requests = create_twin_withdrawal_requests(rng);
...
/// Create Native withdrawal requests and record them.
fn create_and_record_native_withdrawal_requests<const N: usize>(
    transactions: &mut WithdrawalTransactions,
) -> [WithdrawalRequest; N] {
    let requests = create_native_withdrawal_requests();
    for request in &requests {
        transactions.record_withdrawal_request(request.clone());
    }
    requests
...
/// Create ERC20 withdrawal requests and record them.
fn create_and_record_erc20_withdrawal_requests<const N: usize>(
    transactions: &mut WithdrawalTransactions,
) -> [WithdrawalRequest; N] {
    let requests = create_erc20_withdrawal_requests();
    for request in &requests {
        transactions.record_withdrawal_request(request.clone());
    }
    requests
...
/// Create a mix of Native and ERC20 withdrawal requests.
fn create_twin_withdrawal_requests<const N: usize, R: Rng>(rng: &mut R) -> [WithdrawalRequest; N] {
    let mut requests = Vec::with_capacity(N);
    let mut is_native_withdrawals: [bool; N] = [false; N];
    rng.fill(&mut is_native_withdrawals[..]);
    for (i, is_native_withdrawal) in is_native_withdrawals.into_iter().enumerate() {
        let native_ledger_burn_index = LedgerBurnIndex::from(3 * (i as u64));
        let request: WithdrawalRequest = if is_native_withdrawal {
            native_withdrawal_request_with_index(native_ledger_burn_index).into()
        } else {
            let erc20_ledger_burn_index = LedgerBurnIndex::from(7 * (i as u64));
            erc20_withdrawal_request_with_index(native_ledger_burn_index, erc20_ledger_burn_index)
                .into()
        };
...
/// Create Native withdrawal requests.
fn create_native_withdrawal_requests<const N: usize>() -> [WithdrawalRequest; N] {
    let mut requests = Vec::with_capacity(N);
    for i in 1..=N as u64 {
        let native_ledger_burn_index = LedgerBurnIndex::from(3 * i);
        let request = native_withdrawal_request_with_index(native_ledger_burn_index).into();
        requests.push(request);
    }
    requests.try_into().unwrap()
}

/// Create ERC20 withdrawal requests.
fn create_erc20_withdrawal_requests<const N: usize>() -> [WithdrawalRequest; N] {
    let mut requests = Vec::with_capacity(N);
    for i in 1..=N as u64 {
        let native_ledger_burn_index = LedgerBurnIndex::from(3 * i);
        let erc20_ledger_burn_index = LedgerBurnIndex::from(7 * i);
        let request =
            erc20_withdrawal_request_with_index(native_ledger_burn_index, erc20_ledger_burn_index)
                .into();
        requests.push(request);
...
fn create_and_record_transaction<R: Into<WithdrawalRequest>>(
    ...
    transactions.record_created_transaction(withdrawal_request.native_ledger_burn_index(), tx);
    transactions
        .created_tx
        .get_alt(&burn_index)
        .unwrap()
